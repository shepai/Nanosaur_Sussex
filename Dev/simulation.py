...
for gen in range(Generations): #microbial algorithm
    a=agent()
    ...
    while currentT-t<TIME and getBump(a,points)==False: #give 20 seconds for trial
        currentT=time.time()
        current=a.get_image(points) #get radar reading
        action=ag.get_action(current) #get motor control command
        a.set_vector(action[0],action[1]) #set a vector for movement
